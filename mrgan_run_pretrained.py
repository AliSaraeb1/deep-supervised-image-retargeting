  ...
  for name, loader in loaders_dict.items():
    ...
    for x, y, m in loader:
      ...
      for i in range(bs):
        ...
        y_pred_i=torch.clamp(denorm_imagenet(y_pred[i:i+1]), 0.0, 1.0)[0].detach().cpu().permute(1,2,0).numpy()
...
test_loaders={}
ava_test=TIRed(os.path.join(Path,"AVA","test","test_A"), os.path.join(Path,"AVA","test","test_B"), None, image_size=224)
test_loaders["AVA"]=DataLoader(ava_test, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
coco_test=TIRed(os.path.join(Path,"COCO","test","test_A"), os.path.join(Path,"COCO","test","test_B"), None, image_size=224)
test_loaders["COCO"]=DataLoader(coco_test, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
hku_test=TIRed(os.path.join(Path,"HKU-IS","test","test_A"), os.path.join(Path,"HKU-IS","test","test_B"), None, image_size=224)
test_loaders["HKU-IS"]=DataLoader(hku_test, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
wat_test=TIRed(os.path.join(Path,"Waterloo Exploration","test","test_A"), os.path.join(Path,"Waterloo Exploration","test","test_B"), None, image_size=224)
